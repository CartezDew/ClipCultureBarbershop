...
const BookingForm = ({ showMainForm = true }) => {
  ...
  const handleBarberSelect = (barberId) => {
    console.log('🔵 handleBarberSelect called with:', barberId);
    ...
    if (barberId !== 'earliest') {
      ...
      // Scroll to Next button inside modal with improved timing
      console.log('🔵 Setting timeout for scroll (specific barber)...');
      setTimeout(() => {
        console.log('🔵 Timeout fired, calling scrollPopupNextIntoView');
        scrollPopupNextIntoView(100);
      }, 150);
    } else {
      console.log('🔵 "Earliest Available" selected');
      ...
      // Scroll to Next button inside modal with improved timing
      console.log('🔵 Setting timeout for scroll (earliest)...');
      setTimeout(() => {
        console.log('🔵 Timeout fired, calling scrollPopupNextIntoView');
        scrollPopupNextIntoView(100);
      }, 150);
  ...
  useEffect(() => {
    if (popupStep === 2 && formData.barber) {
      console.log('🟠 useEffect scroll trigger for barber selection');
      scrollPopupNextIntoView(0);
    }
  }, [formData.barber, popupStep]);
        ...
        return (
          <div className="booking-step">
            ...
            <div className="form-group">
              <label className="service-label">Choose Your Barber</label>
            </div>
  ...
  const scrollPopupNextIntoView = (delay = 200) => {
    console.log('🟢 scrollPopupNextIntoView called with delay:', delay);
    
    const attempt = () => {
      console.log('🟢 Attempting scroll...');
      
      // Try to find the button by ID first, then fall back to class selectors
      const target = 
        document.getElementById('popup-next-button') ||
        document.getElementById('popup-confirm-button') ||
      ...
      if (!target) {
        console.log('❌ No target found! Aborting scroll.');
        return;
      }

      // Use scrollIntoView on the target element so the browser picks the correct scroll container
      console.log('🟢 Calling scrollIntoView on target');
      try {
        ...
        console.log('✅ scrollIntoView executed on target');
      } catch (error) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        console.log('🟢 Second attempt (150ms)');
        requestAnimationFrame(attempt);
      }, 150);
      setTimeout(() => {
        console.log('🟢 Third attempt (300ms)');
        requestAnimationFrame(attempt);
      }, 300);
    }, delay);
  };
